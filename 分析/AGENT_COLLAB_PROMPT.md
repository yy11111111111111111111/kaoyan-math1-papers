...
integrator 可以对你的会话发出 **interrupt**（`interrupt_session`）。
打断信号**不携带任何内容**——它只是门铃。

**约定：如果你被打断，而打断时没有伴随任何人类指令，
不要把它理解成「停下等人」。它的含义唯一：**
...
这条约定存在的原因：云端会话之间无法直接投递消息，
`interrupt` 是 integrator 唯一能主动触达你的信号，只有 1 bit。
内容全部走命令通道。

**若打断时确实伴随了人类的新指令，以人类指令优先**，
但仍然顺手拉一次命令通道再继续。
